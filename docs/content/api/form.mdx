...
to full fletched form libs, as it is not.

The philosophy of `next-runtime` is to do more on the server, and less on the
client. Simpler form handling is part of that.
...
- **method** _get | post_
  ...
  in name/value pairs, while `post` sends the form-data as an HTTP post
...
- **onSubmit** _(event: FormEvent) => void_
  ...
  `event.preventDefault` to block our handler from being fired.
...
- **shallow** _boolean_
  ...
  Submit the form without updating the current page. The form will still be
  reset, but the `get` handler is not rerun. Defaults to `false`.

## usePendingFormSubmit

A hook that provides access to the data that is currently being submitted. This
allows you to create fancy loading screens.
...
function MyPage() {
  ...
  if (pending) {
    return <p>{`Currently submitting ${pending.data.get('name')}`}</p>;
  }
