...
To allow `POST` requests to include files, you'll need to set the `uploadDir` or
define the `upload` handler. Note that this handler accompanies the `post`
handler. You can have a `post` without an `upload` handler, but an `upload`
without a `post` makes no sense. When neither of the options are provided, file
uploads will be ignored.
