...
The `context` parameter is the object as provided by Next.js [`GetServerSidePropsContext`](https://nextjs.org/docs/basic-features/data-fetching#getserversideprops-server-side-rendering) extended with [cookie](/api/cookies) and [header](/api/headers) handlers as well as query expansion.
...
```shell
curl https://example.com/page-path
# » { name: 'Stephan Meijer', country: 'The Netherlands' }
```

## Handle `POST` Requests

Define the `post` handler to support `POST` requests. Post requests can be used to submit forms to the current page, without the need to create an [`api-route`](https://nextjs.org/docs/api-routes/introduction).
...
The `context` parameter is the object as provided by Next.js [`GetServerSidePropsContext`](https://nextjs.org/docs/basic-features/data-fetching#getserversideprops-server-side-rendering) extended with [cookie](/api/cookies) and [header](/api/headers) handlers, and a populated `req.body`.
...
Note that [`query expansion`](#query-expansion) is applied to the request body as well. A submitted field name like `persons[0].name` translates to a `req.body` like `{ persons: [{ name: '' }]`.
...
To allow `POST` requests to include files, you'll need to set the `uploadDir` or define the `upload` handler. Note that this handler accompanies the `post` handler. You can have a `post` without an `upload` handler, but an `upload` without a `post` makes no sense. When neither of the options are provided, file uploads will be ignored.
...
We use a combination of [`body-parser`](https://www.npmjs.com/package/body-parser) and [`busboy`](https://www.npmjs.com/package/busboy) to parse the incoming data, together with our own glue for `query expansion`.
...
- **mimeType** _string_
  ...
  A comma-separated list of one or more file types or unique file type specifiers to only accept files of certain types. See [MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/accept) for more info. It's the same accept string as `<input type="file" accept="..." />` uses.
...
Our `RuntimeContext` extends `GetServerSidepropsContext` with convenient [cookie](/api/cookies) and [header](/api/headers) helpers.

```ts
export const getServerSideProps = handle<PageProps, UrlQuery, FormData>({
  async upload({ file: File, stream: ReadableStream }): void {},
  async get({ params: UrlQuery }: RuntimeContext): PageProps {},
  async post({ req: { body: FormData } }: RuntimeContext): PageProps {},
});
```
